...
Page({
  ...
  zan:function(event){
    ...
    util.req('addCommentZan','post',{
      'cid':Commentdata[event.currentTarget.id].id,
      'sk':app.globalData.sk
      },function(data){
      if(data.errcode == 0){
        Commentdata[event.currentTarget.id].zan = data.items;
  ...
  shoucang:function(){
    var that = this;
    util.req('addFav','post',{iid:that.data.data.id,sk:app.globalData.sk},function(data){
      if(data.errcode == 0){
  ...
  qxshoucang:function(){
    var that = this;
    util.req('deleteFav','post',{iid:that.data.data.id,sk:app.globalData.sk},function(data){
      if(data.errcode == 0){
  ...
  appointment:function(e){
    ...
    util.req('addAppointment','post',{form_id:fId,iid:this.data.data.id,name:e.detail.value.name,phone:e.detail.value.phone,surplus:e.detail.value.surplus,sk:app.globalData.sk},function(data){
      if(data.errcode == 0){
  ...
  onLoad:function(options){
    ...
    util.req('checkFav', 'post', {iid:options.id,sk:app.globalData.sk},function(data){
      if(data.errcode == 0){
        that.setData({'shoucang':true});
      }
    }) 

    util.req('getInfoDetail', 'get', {iid:options.id},function(data){
      that.setData({data:data.items});
      if(data.items.user.id == app.globalData.userInfo.id){
        var notme = false;
      }else{
        var notme = true;
      }
      var Surpluss = new Array('请选择人数');
      for(var i = 1; i <= data.items.surplus; i++){
        Surpluss.push(i);
      }
      that.setData({
        'data.tm':util.formatTime(new Date(data.items.time*1000)),
        'data.price':(data.items.price == null)?'面议':data.items.price,
        'data.gender':data.items.user.gender,
        'notme':notme,
        'Surpluss':Surpluss,
        'surplus':0
        });
  ...
  getComment:function(id){
    var that = this;
    util.req('getCommentList','get',{iid:id,type:'info',page:page},function(data){
        console.log(data)
      if(data.errcode == 0){
        if(page == 1){          
          comment = new Array();
        ...
        if(!data.items || data.items.length == 0){
          ...
          data.items.forEach(function(item){
            comment.push({
              ...
              fid:item.fid,
              nickName: item.user ? item.user.avatarUrl : '',
  ...
  getCount:function(id){  
    var that = this;  
    util.req('getCommentCount','get', {iid:id,type:'info'},function(data){  //获取评论总数
      if(data.errcode == 0){
        that.setData({comnum:data.items});
      }
    })
  },
  onShow:function(){      
