...
Page({
  ...
  del:function(e){
    ...
    wx.showModal({
      ...
      success: function(res) {
        if (res.confirm) {
          util.req('deleteFav', 'post',{sk:app.globalData.sk,iid:list[currentTarget].id},function(data){
            if(data.errcode == 0){
  ...
  getList(){
    var that = this;
    util.req('getMyFavList','get',{sk:app.globalData.sk,page:page},function(data){
      if(!data.items || data.items.length == 0){
        ...
        that.setData({ list: list, nomore: true});
    })
  },  
